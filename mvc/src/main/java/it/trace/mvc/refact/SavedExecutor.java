...
public class SavedExecutor implements HttpExecutor {

    protected ActionConfig config;
    protected Object action;

    public SavedExecutor(ActionConfig config, Object action) {
        this.config = config;
        this.action = action;
    }
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) {
        try {
            config.getDataBinder().bind(request, response, null, action);
