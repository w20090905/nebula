...
public class PersonDbPersistorDump implements Opcodes {
...
cw.visit(V1_6, ACC_PUBLIC + ACC_SUPER, "nebula/entity/PersonDbPersistor", null, "java/lang/Object", new String[] { "nebula/entity/PersonPersistor" });
...
{
fv = cw.visitField(ACC_FINAL, "cp", "Lnebula/persistor/db/ConnectionProvider;", null, null);
fv.visitEnd();
}
{
mv = cw.visitMethod(ACC_PROTECTED, "<init>", "(Lnebula/persistor/db/ConnectionProvider;Lnebula/persistor/NebulaContext;)V", null, null);
{
...
mv.visitFieldInsn(PUTFIELD, "nebula/entity/PersonDbPersistor", "cp", "Lnebula/persistor/db/ConnectionProvider;");
mv.visitInsn(RETURN);
mv.visitMaxs(2, 3);
mv.visitEnd();
}
...
mv.visitLabel(l0);
mv.visitVarInsn(ALOAD, 0);
mv.visitFieldInsn(GETFIELD, "nebula/entity/PersonDbPersistor", "cp", "Lnebula/persistor/db/ConnectionProvider;");
mv.visitMethodInsn(INVOKEVIRTUAL, "nebula/persistor/db/ConnectionProvider", "get", "()Ljava/sql/Connection;");
mv.visitLdcInsn("select name,sex,height,birthday,company_name,lastModified from person where name = ?");
...
mv.visitFieldInsn(GETFIELD, "nebula/entity/PersonDbPersistor", "cp", "Lnebula/persistor/db/ConnectionProvider;");
mv.visitMethodInsn(INVOKEVIRTUAL, "nebula/persistor/db/ConnectionProvider", "get", "()Ljava/sql/Connection;");
mv.visitLdcInsn("insert into person(name,sex,height,birthday,company_name,lastModified) values(?,?,?,?,?,?)");
mv.visitMethodInsn(INVOKEINTERFACE, "java/sql/Connection", "prepareStatement", "(Ljava/lang/String;)Ljava/sql/PreparedStatement;");
mv.visitVarInsn(ASTORE, 3);
mv.visitInsn(ICONST_0);
mv.visitVarInsn(ISTORE, 2);
...
mv.visitVarInsn(ASTORE, 2);
mv.visitVarInsn(ALOAD, 2);
mv.visitLdcInsn("select name,sex,birthday,company_name,lastModified from person p order by p.lastModified desc \n");
