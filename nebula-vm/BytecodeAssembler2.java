 ...
 */
public class BytecodeAssembler2 extends AssemblerParser {
	...
	protected Map<String, LabelSymbol2> labels = // label scope
	new HashMap<String, LabelSymbol2>();
	/**
	 ...
	 */
	public BytecodeAssembler2(TokenStream lexer, BytecodeDefinition.Instruction[] instructions) {
		super(lexer);
	...
	protected void checkForUnresolvedReferences() {
		for (String name : labels.keySet()) {
			LabelSymbol2 sym = (LabelSymbol2) labels.get(name);
			if (!sym.isDefined) {
	...
	protected int getLabelAddress(String id) {
		LabelSymbol2 sym = (LabelSymbol2) labels.get(id);
		if (sym == null) {
			// assume it's a forward code reference; record opnd address
			sym = new LabelSymbol2(id, ip, true);
			sym.isDefined = false;
	...
	protected void defineLabel(Token idToken) {
		...
		LabelSymbol2 sym = (LabelSymbol2) labels.get(id);
		if (sym == null) {
			LabelSymbol2 csym = new LabelSymbol2(id, ip, false);
