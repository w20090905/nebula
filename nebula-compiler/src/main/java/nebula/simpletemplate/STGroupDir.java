 ...
 * files on-demand. Dir search path: current working dir then CLASSPATH (as a
 * resource). Do not look for templates outside of this dir subtree (except via
 * imports).
 */
public class STGroupDir extends STGroup {
	...
	protected CompiledST load(String name) {
		...
		String parent = Misc.getParent(name); // must have parent; it's
												// fully-qualified
		...
		try {
			...
			return loadTemplateFile(prefix, unqualifiedName + TEMPLATE_FILE_EXTENSION); // load
																						// t.st
																						// file
		} finally { // clean up
			try {
