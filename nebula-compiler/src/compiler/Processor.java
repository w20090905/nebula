...
//            new Processor().processInteractive();
    ...
    throws IOException, RecognitionException {
        ExprParser tokenParser = new ExprParser(getTokenStream(reader));
        ExprParser.prog_return parserResult =
            tokenParser.prog(); // start rule method
        reader.close();
    ...
    throws IOException {
        ExprLexer lexer = new ExprLexer(new ANTLRReaderStream(reader));
        return new CommonTokenStream(lexer);
    }

    // This is public so it can be used by unit tests.
    public void processAST(CommonTree ast) throws RecognitionException {
    	ExprTree treeParser = new ExprTree(new CommonTreeNodeStream(ast));
        treeParser.prog();
    }
