...
[@nl.simple title="${type.displayName} - ${step.displayName}"]
<!-- Start Form -->
	...
	[#list step.type.fields as of][#t][#if !of.internal]
			...
			[@nc.controls field=of for="${of.name}" label="${of.displayName}"]
					[@nc.inputBox field=of id="${of.name}"  ngModel="data.step.${of.name}" placeholder="${of.name}" 
		...
		<legend>${of.name}</legend>
				...
				[@nc.controls field=in1f for="${of.name}${in1f.name}" label="${in1f.displayName}"]
					[@nc.inputBox field=in1f id="${of.name}${in1f.name}" ngModel="data.step.${of.name}.${in1f.name}" placeholder="${of.name} ${in1f.name}"
				...
				[@nc.controls  field=in2f for="${of.name}${in1f.name}${in2f.name}" label="${in1f.displayName}${in2f.displayName}"]
					[@nc.inputBox field=in2f id="${of.name}${in1f.name}${in2f.name}" ngModel="data.step.${of.name}.${in1f.name}${in2f.name}" placeholder="${of.name} ${in1f.name} ${in2f.name}"
				...
				[@nc.controls  field=in1f for="${of.name}${in1f.name}" label="${in1f.displayName}"]
					[@nc.popupBox field=in1f pField=in1f id="${of.name}${in1f.name}" ngModel="data.step.${of.name}.${in1f.name}" placeholder="${of.name} ${in1f.name}"
		...
		<table class="table table-hover table-striped table-bordered table-condensed">
			...
			<tbody>
				<tr x-ng-repeat="inlineData in data.step.${of.name}${in1f.name} | filter:query | orderBy:orderProp">
					<td>#</td>
									[#list in1f.type.fields as in2f][#t]
										[#if !in2f.array && in2f.refer == "ByVal"] <!--  Type   -->

					<td>{{inlinedata.step.${in2f.name}}}</td>
				...
				[@nc.controls field=of for="${of.name}" label="${of.displayName}"]
					[@nc.popupBox field=of pField=of id="${of.name}" ngModel="data.step.${of.name}"  placeholder="${of.name}"
		...
		<table class="table table-hover table-striped table-bordered table-condensed">
			...
			<tbody>
				<tr x-ng-repeat="inlineData in data.step.${of.name} | filter:query | orderBy:orderProp" class="dataline-{{inlineData['_STS']}}">
					<td>{{$index+1}}</td>
					...
					<td>{{inlinedata.step.${in1f.name}}}</td>
					...
					<td>{{inlinedata.step.${in1f.name}${in2f.name}}}</td>
				...
				<tr class="newLine">
					<td>{{data.step.${of.name}.length+1}}</td>
					...
					<td>	[@nc.inputBox field=in1f id="${of.name}${in1f.name}_new"  ngModel="data.step.${of.name}_new.${in1f.name}" placeholder="${of.name} ${in1f.name}" 
					...
					<td>	[@nc.inputBox field=in1f id="${of.name}_new_${in1f.name}${in2f.name}"  ngModel="data.step.${of.name}_new.${in1f.name}${in2f.name}" placeholder="${of.name} ${in1f.name} ${in2f.name}" 
						key=in1f.key required=false/]</td>[#--TODO required=!in1f.ignorable --] 
								...
								[#case "ByRef"]<!--  Type E3-->
								[#case "Cascade"]<!--  Type E4-->										
					<td>[@nc.popupBox field=in1f pField=in1f id="${of.name}_new_${in1f.name}" ngModel="data.step.${of.name}_new.${in1f.name}" placeholder="${of.name} ${in1f.name}"
						key=of.key readonly=true required=false/]</td>[#--  required=!(of.ignorable) --]
									[#break]
								[/#switch]
							[/#if] 
						[/#list]
					<td><a class="btn" x-ng-click="data.step.${of.name}.push(data.step.${of.name}_new);data.step.${of.name}_new={};$event.preventDefault() "> <i class="icon-plus"> </i> </a> </td>
				</tr>
			</tbody>
		</table>
		...
		<div class="form-actions">
			[#list step.type.actions as a][#if !a.internal][#t]
	  		<input type="submit" class="btn btn-primary" x-ng-disabled="form.$invalid"   x-ng-click="$save('${a.name}')"  value="${a.displayName}">
			[/#if][/#list]
