...
jQuery(document).ready(function($) {
	...
	$('#sidebar .submenu:has(ul>li) > a').live("click",function(e)
	{
		...
		} else 
		{
			if(($(window).width() > 768) || ($(window).width() < 479)) {
				...
				submenu.fadeIn(250);
				submenu.find("li").off("click.close").on("click.close",function(){
					submenu.fadeOut(250);
					li.removeClass('open');	
					submenu.find("li").off("click.close");
				});
			}
	...
	$('#sidebar > a').click(function(e)
	{
		...
		} else 
		{
			...
			ul.slideDown(250);
			ul.find("li").not("li:has(ul>li)").off("click.closemain").one("click.closemain",function(){
				ul.slideUp(250);
				sidebar.removeClass('open');
