...
class SimpleEntitySerializer extends DefaultFieldSerializer<Entity> implements JsonDataHelper<Entity> {
	final List<DefaultFieldSerializer<?>> pageFields;
	final Map<String, DefaultFieldSerializer<?>> pageFieldsMap;
	final boolean topLevel;

	public SimpleEntitySerializer(Type type) {
		this(type, null, null);
	}

	public SimpleEntitySerializer(Type type, String fieldName, String frontName) {
	...
	// }
	private void addEntityPageField(String name, String pageFieldName, SimpleEntitySerializer entityMerger) {
		this.pageFields.add(entityMerger);
		this.pageFieldsMap.put(pageFieldName, entityMerger);
	}
	...
	private void init(Type type) {
		for (Field of : type.getFields()) {
			if (!of.isArray()) {
				...
				case Inline: // Type A2
					if (topLevel) {
						SimpleEntitySerializer es = new SimpleEntitySerializer(of.getType(), of.getName(), of.getName());
				...
				case Cascade:
					for (Field inf : of.getType().getFields()) {
						if (!inf.isArray() && inf.getRefer() == Reference.ByVal && (inf.isKey() || inf.isCore())) {
