            ...
            for (Method m : clz.getMethods()) {
                ...
                if (m.getParameterTypes().length == 0 && m.getName().startsWith("get")) {
                    ...
                    if (returnType.isArray()) {
                        log.trace(indent(level) + toFriendlyName(m.getName()) + ">>  [" + returnType.getName() + "]");
                        ...
                        log.trace(indent(level) + toFriendlyName(m.getName()) + ": " + m.invoke(instance).toString());
                        ...
                        if (returnValue == null) {
                            log.trace(indent(level) + toFriendlyName(m.getName()) + ": " + null);
                        } else if (returnValue instanceof String) {
                            log.trace(indent(level) + toFriendlyName(m.getName()) + ": " + (String) returnValue);
                        } else if (returnValue instanceof Class<?>) {
                            log.trace(indent(level) + toFriendlyName(m.getName()) + "[[ Class<" + ((Class<?>) returnValue).getName()
                                    + "> ]]");
...
//                            }
    ...
    static private String toFriendlyName(String methodName){
    	if("get".equals(methodName.subSequence(0, 3))){
